...
public class Trainer {
	...
	private static class TrainerThread implements Callable<Void> {
		TestDataSet.MatrixData data;
		Model nn;
		public TrainerThread(Model nn, TestDataSet.MatrixData data) {
			this.data = data;
			this.nn = nn;
		}
		...
		public Void call() throws Exception {
			try {
				nn.train(data.getE(), data.getX(), data.getY());
			} catch (Exception e) {
	...
	public void run(List<TestDataSet.MatrixData> dataList) {
